...
const { API_RESOURCES_URL, API_COPY_URL, API_MOVE_URL } = require('./constants');
 ...
 * @param {function} [error] Error callback
 */
const copyOrMove = (url, token, from, path, overwrite = false, fields = '', success, error) =>
  request.post({
    url: url,
    token: token,
    query: {
 ...
 * @param {function} [error] Error callback
 */
const copy = (token, from, path, overwrite = false, fields = '', success, error) =>
  copyOrMove(API_COPY_URL, token, from, path, overwrite, fields, success, error);
 ...
 * @param {function} [error] Error callback
 */
const move = (token, from, path, overwrite = false, fields = '', success, error) =>
  copyOrMove(API_MOVE_URL, token, from, path, overwrite, fields, success, error);
