...
/**
 * @callback PublishSuccessCallback
 * @param {Link} data Link to the meta-info about published Resource
 * @param {number} status Response status code
 */
 ...
 * @param {string} token OAuth token
 * @param {string} path File or Folder Path
 * @param {PublishSuccessCallback} [success] Success callback
 * @param {RequestErrorCallback} [error] Error callback
 */
const publish = (token, path, success, error) =>
  request.put(
    {
      url: API_PUBLISH_URL,
      token: token,
 ...
 * @param {RequestErrorCallback} [error] Error callback
 */
const unpublish = (token, path, success, error) =>
  request.put(
    {
