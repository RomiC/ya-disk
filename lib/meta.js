 ...
 * @module ya-disk/meta
 */
import { get as requestGet, patch as requestPatch } from './request.js';

import { API_RESOURCES_URL } from './constants.js';
 ...
 */
export const get = async (token, path, options = {}) =>
  (
    await requestGet({
 ...
 */
export const add = async (token, path, props) =>
  (
    await requestPatch({
      url: API_RESOURCES_URL,
      token,
